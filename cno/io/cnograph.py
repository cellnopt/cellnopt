    ...
    def _get_ranked_agraph(self, rank_method=None):
        ...
        for rank in ranks.keys():
            ...
            if rank == 0:
                ...
                H.add_subgraph(species,  rank='source', name='source', 
                        label='stimuli')
            elif rank == M:
                species = sorted([x for x in ranks[rank] if x not in selfloops])
                H.add_subgraph(species, name="sink", rank='sink')
            else:
                if rank_method in ["same", 'cnor']:
                    # something funky here with self loop
                    # self loop should be ignored here
                    species = sorted([x for x in ranks[rank] if x not in selfloops])
                    H.add_subgraph(species, name=name, rank='same')
        return H
